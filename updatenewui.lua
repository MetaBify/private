...
local UI_TITLE = "Venom Ware - Private"
local LOG_PREFIX = "VENOM-WARE"
local CONFIG_FOLDER = "VenomWare"
...
if bypass_mode == "Automatic" then
	...
	for _ = 1, 100 do
		task.spawn(function()
			error("[" .. LOG_PREFIX .. "]: {ANTICHEAT} | SIMULATING ANTICHEAT FLAGS | (CLIENT)")
			error("[" .. LOG_PREFIX .. "]: {ANTICHEAT} | SIMULATING ANTICHEAT FLAGS | (SERVER)")
		end)
	end
	...
	for k, v in pairs(t1) do
		if v > 90 then
			str = tostring(k)
			print("[" .. LOG_PREFIX .. "]: {ANTICHEAT} | " .. tostring(k) .. " | " .. tostring(v))
		end
	end

	if str then
		warn("[" .. LOG_PREFIX .. "]: {ANTICHEAT} | SUCCESSFULLY INJECTED (VENOM-AC) ON THE GAME ENVIRONMENT | {CLIENT}")
	else
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/MetaBify/libs/refs/heads/main/id.lua"))()
...
local n8 = 1
	...
	local s10 = UI_TITLE .. " V1.2"
	local s11 = CONFIG_FOLDER
	...
	function _Utilities.log(p42, p43, p44)
		warn(string.format("[%s]: {%s} | %s | (%s) ", LOG_PREFIX, p42, p43, p44))
	end
	...
	local s15 = "https://raw.githubusercontent.com/MetaBify/libs/main/"
				...
				(function(p110)
					...
					while true do
						if v754.Position.Y < p110.Y then
							...
							print("[" .. LOG_PREFIX .. "]: {TELEPORT} | Progress: " .. tostring(v757) .. " studs away")
							n17 = 0
						else
							...
							if n17 >= 1 then
								...
								warn("[" .. LOG_PREFIX .. "]: {TELEPORT} | Teleporting...")
				...
				(function(p114)
					...
					while true do
						if not (v769.Position.Y < p114.Y) then
							...
							print("[" .. LOG_PREFIX .. "]: {TELEPORT} | Progress: " .. tostring(v774) .. " studs away")
							n21 = 0
						end
						...
						(function(p115)
							...
							while true do
								if v784.Position.Y < p115.Y then
									...
									print("[" .. LOG_PREFIX .. "]: {TELEPORT} | Progress: " .. tostring(v787) .. " studs away")
									n23 = 0
								else
	...
	local u136 = loadstring(game:HttpGet("https://raw.githubusercontent.com/MetaBify/libs/refs/heads/main/id.lua"))()
	...
	local function u141(p146, p147)
		...
		print("[" .. LOG_PREFIX .. "]: {CRAFT} | Max craft amount: " .. tostring(v522))
		task.wait(0.5)
	...
	v116:AddButton({
		...
		Func = function()
			...
			warn("[" .. LOG_PREFIX .. "]: {ITEMS} | --------------- PICKABLE ITEMS ---------------")

			for k, v in pairs(_workspace.Dropped:GetChildren()) do
				print("[" .. LOG_PREFIX .. "]: {ITEMS} | " .. tostring(k) .. " | " .. tostring(v.Name))
			end
	...
	u176 = hookmetamethod(game, "__namecall", function(p212, ...)
		...
		if v691 == "InvokeServer" then
			...
			if t30[1] == str then
				_Utilities.log("SPOOFED", "SUCCESSFULLY SPOOFED LONE AC WITH VENOM-AC", "CLIENT")
		...
		if v691 == "FireServer" then
			...
			if tostring(p212) == "RemoteEvent" then
				...
				if t31[1] == "Check Player Settings" then
					warn("[" .. LOG_PREFIX .. "]: {REMOTE} | BLOCKED CHECK PLAYER SETTINGS | (CLIENT)")
